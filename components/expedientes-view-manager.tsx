...
const FASE_BADGE: Record<string, { bg: string; text: string }> = {
    RECEPCION: { bg: 'bg-slate-50 border border-slate-200', text: 'text-slate-600' },
    VALORACION: { bg: 'bg-amber-50 border border-amber-100', text: 'text-amber-600' },
    ...
    CIERRE: { bg: 'bg-slate-50 border border-slate-100', text: 'text-slate-400' },
}
...
export default function ExpedientesViewManager({ expedientes, searchQuery }: Props) {
    ...
    return (
        <div className="flex h-[calc(100vh-5rem)] bg-white rounded-xl sm:border border-[#2B463C]/5 shadow-sm overflow-hidden relative">
            ...
            <div className={`w-full sm:w-[360px] flex-shrink-0 border-r border-[#2B463C]/5 flex flex-col bg-[#FAFAF8] ${mobileShowDetail ? 'hidden sm:flex' : 'flex'}`}>
                {/* Header */}
                <div className="px-4 pt-4 pb-2">
                    <div className="flex items-center justify-between mb-3">
                        <h2 className="text-lg font-bold text-[#2B463C]">Expedientes</h2>
                        <Link
                            href="/dashboard/recepcion"
                            className="text-[10px] font-semibold text-white bg-[#2B463C] hover:bg-[#F28C73] px-3 py-1.5 rounded-lg transition-colors"
                        >
                            + Nuevo
                        </Link>
                    </div>
                    ...
                    <div className="relative mb-3">
                        ...
                        />
                        {localSearch && (
                            <button onClick={() => setLocalSearch('')} className="absolute right-2 top-1/2 -translate-y-1/2 p-0.5 text-[#2B463C]/30 hover:text-[#2B463C]">
                    ...
                    {filtered.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-16 px-6 text-center">
                            <div className="w-12 h-12 rounded-full bg-[#F8F5EE] flex items-center justify-center mb-3">
                                <FolderHeart className="w-5 h-5 text-[#2B463C]/15" />
                            </div>
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="relative flex-shrink-0">
                                        <div className="w-11 h-11 rounded-full bg-[#F8F5EE] flex items-center justify-center">
                                            <FolderHeart className="w-5 h-5 text-[#F28C73]/60" />
                                        </div>
                ...
                {!selected ? (
                    <div className="flex-1 flex flex-col items-center justify-center text-center px-8">
                        <div className="w-20 h-20 rounded-full bg-[#F8F5EE] flex items-center justify-center mb-4">
                            <FolderHeart className="w-8 h-8 text-[#2B463C]/10" />
                        </div>
                        <h3 className="text-lg font-semibold text-[#2B463C]/60 mb-1">Selecciona un expediente</h3>
                    ...
                    <>
                        ...
                        <div className="border-b border-[#2B463C]/5 px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between bg-[#FAFAF8]">
                            {/* Mobile Back */}
                            <button onClick={handleBack} className="sm:hidden flex items-center gap-1 text-[#2B463C]/60 hover:text-[#2B463C] mr-3 -ml-1 p-1">
                                <ArrowLeft className="w-5 h-5" />
                            </button>

                            <div className="flex items-center gap-4 flex-1 min-w-0">
                                <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-[#F8F5EE] flex items-center justify-center flex-shrink-0">
                                    <FolderHeart className="w-6 h-6 text-[#F28C73]" />
                                </div>
                                <div className="min-w-0">
                                    <h2 className="text-base sm:text-lg font-bold text-[#2B463C] truncate">{selected.radicado}</h2>
                                    <p className="text-xs sm:text-sm text-[#2B463C]/40 truncate">
                            ...
                            <Link
                                ...
                                className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-[#2B463C] text-white text-xs sm:text-sm font-medium rounded-lg hover:bg-[#F28C73] transition-colors shadow-sm flex-shrink-0"
                            >
                        ...
                        <div className="flex-1 p-4 sm:p-6 overflow-y-auto">
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 max-w-4xl">
                                ...
                                <div className="bg-[#FAFAF8] rounded-xl p-5 border border-[#2B463C]/5">
                                    <h3 className="text-sm font-semibold text-[#2B463C] mb-4 flex items-center gap-2">
                                        <Scale className="w-4 h-4 text-[#F28C73]" />
                                        Estado del Proceso
                                    </h3>
                                    <div className="space-y-3">
                                        <div>
                                            <p className="text-[11px] text-[#2B463C]/35 font-medium mb-1">Fase actual</p>
                                            {(() => {
                                                const fase = selected.fase_proceso || 'RECEPCION'
                                                const s = FASE_BADGE[fase] || FASE_BADGE.RECEPCION
                                                return (
                                    ...
                                    {selectedVictima ? (
                                        <div className="space-y-3">
                                            ...
                                            <Link
                                                ...
                                                className="inline-flex items-center gap-1.5 text-xs text-[#F28C73] hover:text-[#2B463C] font-medium mt-1 transition-colors"
                                            >
                                                Ver perfil completo <ArrowRight className="w-3.5 h-3.5" />
                                            </Link>
                                        </div>
                                    ) : (
                                        <p className="text-sm text-[#2B463C]/30">Sin víctima registrada</p>
                                    )}
                                </div>
                                ...
                                <div className="bg-[#FAFAF8] rounded-xl p-5 border border-[#2B463C]/5">
                                    ...
                                    {selectedAgresor ? (
                                        ...
                                        <p className="text-sm text-[#2B463C]/30">Sin agresor registrado</p>
                                    )}
                                </div>
                            </div>
                        </div>
