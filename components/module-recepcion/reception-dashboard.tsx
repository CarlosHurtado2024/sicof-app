        ...
        <div className={`transition-colors duration-500 ${isCrisisMode ? 'bg-red-50/30 p-2 sm:p-4 rounded-xl' : ''}`}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-10">
                ...
                <section className={`lg:col-span-6 ${isCrisisMode ? 'opacity-50 pointer-events-none filter grayscale' : ''}`}>
                    <div className="bg-white rounded-2xl border border-slate-100/80 overflow-hidden h-full flex flex-col shadow-sm">
                                ...
                                <tbody className="divide-y divide-slate-50">
                                    {filteredMinutas.length === 0 ? (
                                        ...
                                        filteredMinutas.map((minuta) => (
                                            <tr key={minuta.id} className="hover:bg-slate-50 transition-all duration-200 group">
                                                ...
                                                <td className="px-6 py-5">
                                                    <div className="flex items-center">
                                                        <div className="w-9 h-9 rounded-full bg-slate-100 text-slate-600 flex items-center justify-center font-bold text-xs mr-3 border border-white shadow-sm">
                                                            {minuta.nombre_visitante.split(' ').map(n => n[0]).join('').substring(0, 2)}
                                                        </div>
                                                        <div>
                                                            <p className="text-sm font-bold text-slate-900">{minuta.nombre_visitante}</p>
                                                ...
                                                <td className="px-6 py-5">
                                                    {minuta.fecha_hora_salida ? (
                                                        <div className="flex items-center">
                                                            ...
                                                            <span className="text-sm text-slate-500 font-bold uppercase tracking-tight">Completado</span>
                                                        </div>
                                                    ) : (
                                                        <div className="flex items-center">
                                                            <span className="h-1.5 w-1.5 rounded-full bg-emerald-500 mr-2.5 animate-pulse" />
                                                            <span className="text-sm text-slate-700 font-bold uppercase tracking-tight">En Proceso</span>
                                                        </div>
                                                    )}
                                                </td>
                                            </tr>
