...
const RIESGO_CONFIG: Record<string, { bg: string; text: string; label: string }> = {
    ...
    ALTO: { bg: 'bg-orange-100', text: 'text-orange-300', label: 'Alto' },
    CRITICO: { bg: 'bg-red-100', text: 'text-red-300', label: 'Crítico' },
}
...
export default async function CasoDetallePage({ params }: { params: Promise<{ id: string }> }) {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start gap-4">
                <div>
                    <div className="flex items-center gap-3">
                        <h1 className="text-2xl font-bold text-white/90">Expediente {expediente.radicado}</h1>
            ...
            {/* === STEPPER DE 5 FASES === */}
            <Card className="border-0 shadow-[0_0_20px_rgba(0,0,0,0.3)] overflow-hidden bg-white/[0.02] backdrop-blur-3xl">
                                ...
                                <div key={fase} className="flex-1 relative min-w-[80px]">
                                    ...
                                    {i < FASES_ORDEN.length - 1 && (
                                        <div className="absolute top-7 right-0 translate-x-1/2 z-10">
                                            <ChevronRight className={`h-4 w-4 ${completada ? 'text-emerald-400' : 'text-white/30'}`} />
                                        </div>
                                    )}
                                </div>
                            )
                                ...
                                return (
                                    <div key={fase} className="flex items-stretch gap-3">
                                        ...
                                        <div className="flex flex-col items-center">
                                            ...
                                            {!isLast && (
                                                <div className={`w-0.5 flex-1 min-h-[12px] ${completada ? 'bg-emerald-300' : 'bg-white/10'}`} />
                                            )}
                                        </div>
                                        {/* Phase label */}
                ...
                <div className="grid gap-3 md:grid-cols-2">
                    {expediente.terminos_legales.map((termino: any) => {
                        ...
                        const semaforoColors = {
                            VERDE: 'border-emerald-200 bg-emerald-500/10 border border-emerald-500/20',
                            AMARILLO: 'border-amber-200 bg-amber-500/10 border border-amber-500/20',
                            ROJO: 'border-red-200 bg-red-500/10 border border-red-500/20'
                        }
                        const semaforoText = {
                            VERDE: 'text-emerald-300',
                        ...
                        return (
                            <div key={termino.id} className={`p-3 rounded-lg border ${semaforoColors[semaforo]} flex items-center gap-3`}>
                                <div className={`p-2 rounded-full ${semaforo === 'ROJO' ? 'bg-red-200' : semaforo === 'AMARILLO' ? 'bg-amber-200' : 'bg-emerald-200'}`}>
                                    {semaforo === 'ROJO' ? <AlertTriangle className="h-4 w-4 text-red-300" /> : <Clock className="h-4 w-4" />}
                                </div>
                                <div className="flex-1">
                                    <p className={`text-sm font-semibold ${semaforoText[semaforo]}`}>{termino.tipo.replace(/_/g, ' ')}</p>
                                    <p className="text-xs text-white/50">{termino.cumplido ? '✅ Cumplido' : tiempoRestante}</p>
                                </div>
                            </div>
                        )
                    })}
            ...
            <div className="grid gap-6 lg:grid-cols-3">
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    {/* FASE 1: RECEPCIÓN */}
                    <Card className="border-0 shadow-[0_0_20px_rgba(0,0,0,0.3)] bg-white/[0.02] backdrop-blur-3xl">
                        <CardHeader className="border-b border-white/10 bg-white/[0.03] backdrop-blur-md">
                        ...
                        <CardContent className="p-4 space-y-4">
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                                <div>
                                    <p className="text-white/40 text-xs uppercase tracking-wide">Radicado</p>
                                    <p className="font-mono font-bold text-purple-300">{expediente.radicado}</p>
                                </div>
                                <div>
                                    <p className="text-white/40 text-xs uppercase tracking-wide">Fecha Apertura</p>
                                    <p>{new Date(expediente.created_at).toLocaleDateString('es-CO')}</p>
                                </div>
                                <div>
                                    <p className="text-white/40 text-xs uppercase tracking-wide">Tipología</p>
                                    <p className="font-medium">{expediente.tipologia_violencia}</p>
                                </div>
                                <div>
                                    <p className="text-white/40 text-xs uppercase tracking-wide">Competencia</p>
                                ...
                                {victima && (
                                    <div className="p-3 bg-purple-500/10 border border-purple-500/20 rounded-lg">
                                        <div className="flex items-center gap-2 mb-2">
                                            ...
                                            <p className="text-xs font-bold text-purple-300 uppercase">Víctima</p>
                                        </div>
                                        <p className="font-semibold text-white/90">{victima.nombres}</p>
                                        <p className="text-xs text-white/50">{victima.tipo_documento}: {victima.documento}</p>
                                        {victima.telefono && <p className="text-xs text-white/50">Tel: {victima.telefono}</p>}
                                    </div>
                                )}
                                {agresor && (
                                    <div className="p-3 bg-red-500/10 border border-red-500/20 rounded-lg">
                                        <div className="flex items-center gap-2 mb-2">
                                            ...
                                            <p className="text-xs font-bold text-red-300 uppercase">Agresor</p>
                                        </div>
                                        <p className="font-semibold text-white/90">{agresor.nombres}</p>
                                        <p className="text-xs text-white/50">Doc: {agresor.documento}</p>
                            ...
                            {expediente.hechos_relato && (
                                <div>
                                    <p className="text-white/40 text-xs uppercase tracking-wide mb-1">Relato de Hechos</p>
                                    <p className="text-sm text-white/80 bg-white/5 p-3 rounded">{expediente.hechos_relato}</p>
                                </div>
                            )}
                        </CardContent>
                    </Card>
                    ...
                    <Card className="border-0 shadow-[0_0_20px_rgba(0,0,0,0.3)] bg-white/[0.02] backdrop-blur-3xl">
                        ...
                        <CardContent className="p-4">
                            {(!expediente.valoraciones_riesgo || expediente.valoraciones_riesgo.length === 0) ? (
                                <p className="text-sm text-white/40 py-4 text-center">No se han realizado valoraciones de riesgo.</p>
                            ) : (
                                <div className="space-y-3">
                                    {expediente.valoraciones_riesgo.map((val: any) => (
                                        <div key={val.id} className="flex items-center gap-3 p-3 bg-white/5 rounded-lg">
                                            ...
                                            <div className="flex-1">
                                                <p className="text-sm font-medium text-white/80">{val.recomendacion_medidas || 'Sin observaciones'}</p>
                                                <p className="text-xs text-white/40">{new Date(val.fecha_valoracion).toLocaleString('es-CO')}</p>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                    ...
                    <Card className="border-0 shadow-[0_0_20px_rgba(0,0,0,0.3)] bg-white/[0.02] backdrop-blur-3xl">
                        ...
                        <CardContent className="p-4 space-y-4">
                            {/* Medidas */}
                            <div>
                                <p className="text-xs font-bold text-white/50 uppercase mb-2">Medidas Dictadas</p>
                                {(!expediente.medidas || expediente.medidas.length === 0) ? (
                                    <p className="text-sm text-white/40 text-center py-3">No se han dictado medidas.</p>
                                ) : (
                                        ...
                                        {expediente.medidas.map((m: any) => (
                                            <div key={m.id} className="flex items-center gap-3 p-3 bg-white/5 rounded-lg">
                                                ...
                                                <div className="flex-1">
                                                    ...
                                                    {m.descripcion && <p className="text-xs text-white/50">{m.descripcion}</p>}
                                                </div>
                                                <span className={`text-xs px-2 py-1 rounded-full ${m.estado === 'VIGENTE' ? 'bg-emerald-100 text-emerald-300' : 'bg-white/10 text-white/70'}`}>
                                                    {m.estado}
                                                </span>
                                            </div>
                                        ))}
                            ...
                            {/* Audiencias */}
                            <div>
                                <p className="text-xs font-bold text-white/50 uppercase mb-2">Audiencias</p>
                                {(!expediente.audiencias || expediente.audiencias.length === 0) ? (
                                    <p className="text-sm text-white/40 text-center py-3">No se han programado audiencias.</p>
                                ) : (
                                    <div className="space-y-2">
                                        {expediente.audiencias.map((a: any) => (
                                            <div key={a.id} className="flex items-center gap-3 p-3 bg-white/5 rounded-lg">
                                                ...
                                                <span className={`text-xs px-2 py-1 rounded-full ${a.estado === 'PROGRAMADA' ? 'bg-purple-100 text-purple-300' : a.estado === 'REALIZADA' ? 'bg-emerald-100 text-emerald-300' : 'bg-amber-100 text-amber-300'}`}>
                                                    {a.estado}
                                                </span>
                                            </div>
                                        ))}
                    ...
                    <Card className="border-0 shadow-[0_0_20px_rgba(0,0,0,0.3)] bg-white/[0.02] backdrop-blur-3xl">
                        ...
                        <CardContent className="p-4">
                            {(!expediente.seguimientos || expediente.seguimientos.length === 0) ? (
                                <p className="text-sm text-white/40 text-center py-3">No hay seguimientos registrados.</p>
                            ) : (
                                <div className="space-y-2">
                                    {expediente.seguimientos.map((s: any) => (
                                        <div key={s.id} className="flex items-center gap-3 p-3 bg-white/5 rounded-lg">
                                            ...
                                            <div className="flex-1">
                                                ...
                                                {s.observaciones && <p className="text-xs text-white/50 mt-1">{s.observaciones}</p>}
                                            </div>
                                            <span className={`text-xs px-2 py-1 rounded-full ${s.cumplimiento ? 'bg-emerald-100 text-emerald-300' : s.fecha_realizada ? 'bg-red-100 text-red-300' : 'bg-white/10 text-white/70'}`}>
                            ...
                            {(!expediente.incidentes_incumplimiento || expediente.incidentes_incumplimiento.length === 0) ? (
                                ...
                                <div className="space-y-2">
                                    {expediente.incidentes_incumplimiento.map((inc: any) => (
                                        <div key={inc.id} className="p-3 bg-red-500/10 border border-red-500/20 rounded-lg border border-red-200">
                                            <p className="text-sm font-semibold text-red-800">Incidente de Incumplimiento</p>
                                            <p className="text-sm text-red-300 mt-1">{inc.descripcion_incumplimiento}</p>
                ...
                <div className="space-y-6">
                    <Card className="border-0 shadow-[0_0_20px_rgba(0,0,0,0.3)] bg-white/[0.02] backdrop-blur-3xl">
                        ...
                        <CardContent className="p-4">
                            {(!expediente.actuaciones || expediente.actuaciones.length === 0) ? (
                                <p className="text-sm text-white/40 text-center py-3">Sin actuaciones registradas.</p>
                            ) : (
                                <div className="space-y-3">
                                    {expediente.actuaciones
                                        .sort((a: any, b: any) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())
                                        .map((act: any) => (
                                            <div key={act.id} className="relative pl-4 border-l-2 border-white/10 pb-3">
                                                <div className="absolute -left-[5px] top-1 w-2 h-2 rounded-full bg-purple-500" />
                                                <p className="text-xs text-white/40">{new Date(act.created_at).toLocaleString('es-CO')}</p>
                                                <p className="text-sm font-medium text-white/80">{act.tipo?.replace(/_/g, ' ')}</p>
                                                {act.contenido && <p className="text-xs text-white/50 mt-1 line-clamp-2">{act.contenido}</p>}
                                            </div>
                                        ))}
                                </div>
                            )}
                    ...
                    <Card className="border-0 shadow-[0_0_20px_rgba(0,0,0,0.3)] bg-white/[0.02] backdrop-blur-3xl">
                        ...
                        <CardContent className="p-4 space-y-3 text-sm">
                            ...
                            <div className="flex justify-between">
                                <span className="text-white/50">Nivel Riesgo</span>
                            ...
                            <div className="flex justify-between">
                                <span className="text-white/50">Actuaciones</span>
                                <span className="font-medium">{expediente.actuaciones?.length || 0}</span>
                            </div>
                            <div className="flex justify-between">
                                <span className="text-white/50">Medidas</span>
