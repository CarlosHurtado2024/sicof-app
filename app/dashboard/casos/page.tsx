...
const RIESGO_CONFIG: Record<string, { bg: string; text: string; dot: string; label: string }> = {
    SIN_RIESGO: { bg: 'bg-slate-50 border border-slate-100', text: 'text-slate-500', dot: 'bg-slate-400', label: 'Sin Riesgo' },
    ...
    ALTO: { bg: 'bg-orange-50 border border-orange-100', text: 'text-orange-700', dot: 'bg-orange-500', label: 'Alto' },
...
const FASE_CONFIG: Record<string, { bg: string; text: string; dot: string }> = {
    RECEPCION: { bg: 'bg-blue-50 border border-blue-100', text: 'text-blue-700', dot: 'bg-blue-600' },
    VALORACION: { bg: 'bg-slate-100 border border-slate-200', text: 'text-slate-700', dot: 'bg-slate-600' },
    ...
    CIERRE: { bg: 'bg-slate-50 border border-slate-100', text: 'text-slate-500', dot: 'bg-slate-400' },
}
        ...
        <div className="space-y-6 max-w-[1400px] mx-auto">
            ...
            <div className="animate-fade-in-up">
                <div className="flex items-center space-x-2 text-slate-400 text-sm mb-3 font-medium">
                    ...
                    <span className="text-blue-600 font-bold">Expedientes</span>
                </div>
                    ...
                    <div>
                        <h1 className="text-3xl font-black tracking-tight text-slate-900 flex items-center gap-4">
                            <div className="p-3 bg-blue-100 rounded-2xl shadow-sm">
                                <FolderHeart className="h-6 w-6 text-blue-700" />
                            </div>
                            Gestión de Expedientes
                        </h1>
                        <p className="text-slate-500 text-sm mt-2 ml-1 font-medium italic">Control y seguimiento de la Ruta de Atención Integral</p>
                    </div>
                    <div className="flex items-center gap-3">
                        <Link href="/dashboard/recepcion/nuevo-caso">
                            <Button className="bg-blue-900 hover:bg-slate-800 text-white gap-2 px-8 py-6 rounded-xl font-bold shadow-lg transition-all active:scale-95">
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
                <Suspense fallback={
                    <div className="flex-1 max-w-md h-12 bg-slate-100 rounded-xl animate-pulse" />
                }>
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="bg-slate-50 border-b border-slate-200">
                                    <th className="text-left px-6 py-4 font-black text-slate-400 text-[10px] uppercase tracking-[0.2em]">Radicado</th>
                                    <th className="text-left px-6 py-4 font-black text-slate-400 text-[10px] uppercase tracking-[0.2em]">Causa / Víctima</th>
                                    <th className="text-left px-6 py-4 font-black text-slate-400 text-[10px] uppercase tracking-[0.2em]">Tipología</th>
                                    <th className="text-left px-6 py-4 font-black text-slate-400 text-[10px] uppercase tracking-[0.2em]">Fase Actual</th>
                                    <th className="text-left px-6 py-4 font-black text-slate-400 text-[10px] uppercase tracking-[0.2em]">Nivel Riesgo</th>
                                    <th className="text-left px-6 py-4 font-black text-slate-400 text-[10px] uppercase tracking-[0.2em]">Apertura</th>
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {(!expedientes || expedientes.length === 0) ? (
                                    ...
                                    expedientes.map((exp: any) => {
                                        ...
                                        return (
                                            <tr key={exp.id} className="hover:bg-slate-50 transition-all duration-300 group">
                                                <td className="px-6 py-5">
                                                    <span className="font-mono font-black text-blue-700 text-xs bg-blue-50 border border-blue-100 px-3 py-1.5 rounded-xl shadow-sm">{exp.radicado}</span>
                                                </td>
                                                <td className="px-6 py-5">
                                                    <div className="flex items-center gap-4">
                                                        <div className="w-10 h-10 rounded-xl bg-slate-100 flex items-center justify-center text-xs font-black text-slate-400 flex-shrink-0 shadow-inner group-hover:bg-white transition-colors">
                                                            {victima?.nombres ? victima.nombres.split(' ').map((n: string) => n[0]).join('').substring(0, 2).toUpperCase() : '??'}
                                                        </div>
                                                        <div className="min-w-0">
                                                            <p className="font-bold text-slate-800 text-sm group-hover:text-blue-700 transition-colors truncate">{victima?.nombres || '—'}</p>
                                                            <p className="text-[11px] text-slate-400 font-bold uppercase tracking-tighter italic">{victima?.documento || ''}</p>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td className="px-6 py-5">
                                                    <span className="text-xs text-slate-600 font-bold bg-slate-50 px-2 py-1 rounded-lg border border-slate-100">{exp.tipologia_violencia || '—'}</span>
                                                </td>
                                                <td className="px-6 py-5">
                                                    <span className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-wider shadow-sm ${faseConfig.bg} ${faseConfig.text}`}>
                                                        <span className={`w-2 h-2 rounded-full ${faseConfig.dot} animate-pulse`}></span>
                                                        {FASES_INFO[faseKey]?.nombre || faseKey}
                                                    </span>
                                                </td>
                                                <td className="px-6 py-5">
                                                    <span className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-wider shadow-sm ${riesgoConfig.bg} ${riesgoConfig.text}`}>
                                                        <span className={`w-2 h-2 rounded-full ${riesgoConfig.dot}`}></span>
                                                        {riesgoConfig.label}
                                                    </span>
                                                </td>
                                                ...
                                                <td className="px-6 py-5 text-right">
                                                    <Link href={`/dashboard/casos/${exp.id}`}>
                                                        <Button
                                                            variant="ghost"
                                                            size="sm"
                                                            className="h-9 gap-2 px-4 text-blue-700 hover:text-white hover:bg-blue-900 rounded-xl font-bold text-[11px] uppercase tracking-wider transition-all"
                                                        >
                                                            <Eye size={14} />
                                                            Expediente
                                                        </Button>
